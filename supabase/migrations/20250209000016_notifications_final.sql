-- ============================================
-- SYSTÈME DE NOTIFICATIONS - VERSION FINALE
-- Compatible avec structure existante (sans table companies)
-- ============================================
...
-- Table notifications (sans FK vers companies pour compatibilité)
CREATE TABLE IF NOT EXISTS notifications (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
    company_id UUID, -- Pas de FK, juste un UUID pour compatibilité
...
DECLARE
    ...
    days_until INTEGER;
BEGIN
    -- Vérifier si next_service_date existe et a une valeur
    ...
    LOOP
        INSERT INTO notifications (
            ...
            v_user.id,
            NULL, -- company_id non géré dans cette version
            'maintenance_due',
...
BEGIN
    ...
    IF v_table = 'drivers' AND column_exists('drivers', 'license_expiry') THEN
...
-- ============================================
-- PARTIE 8: ACTIVER REALTIME
-- ============================================
DO $$
BEGIN
    -- Vérifier si la publication existe
    IF EXISTS (SELECT 1 FROM pg_publication WHERE pubname = 'supabase_realtime') THEN
        -- Ajouter la table notifications à la publication
        ALTER PUBLICATION supabase_realtime ADD TABLE notifications;
    END IF;
EXCEPTION WHEN OTHERS THEN
    -- Ignorer si déjà ajoutée ou autre erreur
    NULL;
END $$;
