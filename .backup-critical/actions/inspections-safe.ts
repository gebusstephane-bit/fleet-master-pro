...
export async function validateInspection(inspectionId: string) {
  ...
  try {
    ...
    if (inspectionCompanyId !== profile.company_id) {
      console.warn(`[IDOR] Tentative d'accès interdit: user ${user.id} (company: ${profile.company_id}) tente de valider l'inspection ${inspectionId} (company: ${inspectionCompanyId})`);
      return { error: 'Inspection non trouvée', data: null };
    }
...
export async function rejectInspection(inspectionId: string, reason: string) {
  ...
  try {
    ...
    const inspectionCompanyId = inspection.company_id;
    if (!inspectionCompanyId) {
      console.error('[rejectInspection] Inspection sans company_id:', inspectionId);
      return { error: 'Inspection invalide (pas de company_id)', data: null };
    }
    
    if (inspectionCompanyId !== profile.company_id) {
      console.warn(`[IDOR] Tentative d'accès interdit: user ${user.id} (company: ${profile.company_id}) tente de rejeter l'inspection ${inspectionId} (company: ${inspectionCompanyId})`);
      return { error: 'Inspection non trouvée', data: null };
    }
