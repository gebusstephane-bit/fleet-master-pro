...
async function handleNewRegistration(
  ...
  try {
    ...
    let planType: PlanType = 'ESSENTIAL'; // défaut
    ...
    if (setupToken) {
      ...
      if (findError || !pending) {
        ...
        console.log('✅ Token valide trouvé, données récupérées localement');
      }
    }
    
    // Si token invalide ou manquant, générer un mot de passe aléatoire sécurisé
    if (!tokenValid) {
      console.warn('⚠️ Token invalide/expiré - Génération mot de passe aléatoire + email récupération');
    ...
    if (tokenValid && pendingReg) {
      ...
      if (updateError) {
        console.error('⚠️ Erreur mise à jour token (non bloquant):', updateError);
      } else {
    ...
    try {
      if (!tokenValid) {
        ...
        console.log('📧 Envoi email récupération (token expiré)');
    ...
    console.log(`✅ User created after payment: ${email} -> ${plan} (token valid: ${tokenValid})`);
