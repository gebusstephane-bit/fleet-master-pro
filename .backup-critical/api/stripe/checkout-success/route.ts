...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('🔍 Vérification utilisateur pour:', email);
    ...
    while (attempts < 10) {
      ...
      if (profile) {
        userProfile = profile;
        console.log('✅ Utilisateur trouvé (créé par webhook):', profile.id);
        break;
      }
    ...
    if (!userProfile) {
      ...
      console.log('🔑 Setup token trouvé:', setupToken);
      ...
      console.log('✅ Données pending_registrations trouvées');
    ...
    console.log('✅ Redirection vers page de succès pour:', userProfile.email);
