...
export async function POST(request: NextRequest) {
  try {
    ...
    if (vehicle.company_id !== profile.company_id) {
      console.warn(`[SOS V3.2] Tentative IDOR détectée: user ${user.id} tente d'accéder au véhicule ${vehicleId} d'une autre entreprise`);
      return NextResponse.json({ error: 'Véhicule non trouvé' }, { status: 404 });
    }
...
async function standardGarageSearch(
  ...
  const partnersWithDistance = (partners || [] as ServiceProvider[])
    .map((p) => {
      const dist = coordinates ? calculateDistance(coordinates, { lat: p.lat, lng: p.lng }) : null;
      return { ...p, distance_km: dist };
    })
    .filter((p: ServiceProvider & { distance_km: number | null }) => p.distance_km === null || p.distance_km <= (p.intervention_radius_km || 50))
    .sort((a: ServiceProvider & { distance_km: number | null }, b: ServiceProvider & { distance_km: number | null }) => (a.distance_km || 999) - (b.distance_km || 999));
